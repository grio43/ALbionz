...
{
    public class FactionWarfareComplexController : BaseController
    {
        #region Constructors

        public FactionWarfareComplexController()
        {
            IgnorePause = false;
            IgnoreModal = false;
        ...
        public static void ProcessState()
        {
            ...
            FactionWarfareComplexBehavior.ProcessState();
        }
        ...
        public override void DoWork()
        {
            try
            {
                ...
                {
                    if (DebugConfig.DebugCombatMissionsBehavior) Log("FactionWarfareComplexBehavior: Loading Settings");
                    ...
                    if (Settings.CharacterXmlExists)
                    {
                        if (DateTime.UtcNow > Time.Instance.NextStartupAction)
                        {
                            try
                            {
                                ...
                                {
                                    if (DebugConfig.DebugInteractWithEve) Log("FactionWarfareComplexBehavior: RunOnce: OpenCargoHold: !OkToInteractWithEveNow");
                                    return;
                                }

                                if (DebugConfig.DebugCombatMissionsBehavior) Log("FactionWarfareComplexBehavior: RunOnce");
                ...
                {
                    if (DebugConfig.DebugAbyssalDeadspaceBehavior) Log("FactionWarfareComplexBehavior: if (ESCache.Instance.InWarp)");
                    return;
                }

                if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)
                {
                    Log("FactionWarfareComplexBehavior: if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)");
                ...
                {
                    Log("FactionWarfareComplexBehavior: if (ESCache.Instance.InStation && ESCache.Instance.DeactivateScheduleAndCloseAfterNextDock)");
