...
{
    public class GatherItemsController : BaseController
    {
        #region Constructors

        public GatherItemsController()
        {
            IgnorePause = false;
            IgnoreModal = false;
            Time.Instance.NextStartupAction = DateTime.UtcNow;
            Time.Instance.Started_DateTime = DateTime.UtcNow;
            AbyssalDeadspaceBehaviorInstance = new AbyssalDeadspaceBehavior();
        }
        ...
        private AbyssalDeadspaceBehavior AbyssalDeadspaceBehaviorInstance { get; }
        ...
        {
            // Gather Items Behavior
            GatherItemsBehavior.ProcessState();
        }
        ...
        public override void DoWork()
        {
            try
            {
                ...
                {
                    if (DebugConfig.DebugAbyssalDeadspaceBehavior) Log("AbyssalDeadspaceController: if (ESCache.Instance.InWarp)");
                    return;
                }

                if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)
                {
                    if (DebugConfig.DebugAbyssalDeadspaceBehavior) Log("AbyssalDeadspaceController: if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)");
