...
namespace EVESharpCore.Controllers
{
    public class GateDecloakController : BaseController
    {
        ...
        static ActionQueueAction action = new ActionQueueAction(null, () =>
        {
            ...
            var activeShipPosition = activeShip.Entity.PositionInSpace;
            ...
            for (var time = 0d; time <= MaxTimeCalculated; time += IntervalSteps)
            {
                ...
                var targetDistanceTravelled = GetDistanceTravelled(
                    ...
                    targetAgility,
                    targetMass);
                var targetPostionAtTime = cloakyHauler.PositionInSpace + (targetAlignment.Normalize() * targetDistanceTravelled);
                ...
                try
                {
                    Vec3 ourAlignment = ((Vec3)targetPostionAtTime - (Vec3)activeShipPosition).Normalize();
                    Vec3 ourPositionAtTime = (Vec3)activeShipPosition + ((Vec3)ourAlignment * ourDistanceTravelled);

                    var currentCollisionDistance = ((Vec3)ourPositionAtTime - (Vec3)targetPostionAtTime).Magnitude;

                    if (currentCollisionDistance < closestCollisionDistance)
                    {
                        closestCollisionDistance = currentCollisionDistance;
                        closestCollisionPosition = (Vec3)targetPostionAtTime;
                    }
                    else
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    Log($"cloakyHauler.Position {cloakyHauler.PositionInSpace} targetAlignment {targetAlignment} targetDistanceTravelled {targetDistanceTravelled}");
            ...
            Vec3 directionalVectorToTarget = (Vec3)closestCollisionPosition - (Vec3)activeShipPosition;
            framework.SceneManager.DrawLine(new Vec3(0, 0, 0), (Vec3)directionalVectorToTarget);
        ...
        public override void DoWork()
        {
            ...
            var mwd = ESCache.Instance.DirectEve.Modules.FirstOrDefault(e => e.GroupId == (int)Group.Afterburner);
