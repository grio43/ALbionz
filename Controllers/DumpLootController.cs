    ...
    public class DumpLootController : BaseController
    {
        ...
        private bool AnyJitaUndockBookmark
        {
            get
            {
                ...
                return ESCache.Instance.CachedBookmarks.Where(i => i.BookmarkType == BookmarkType.Coordinate).Any(b => b.DistanceTo(jita) < 150000);
            }
        }
        ...
        public static bool ShouldDumpLoot
        {
            get
            {
                ...
                if (_disabledForThisSession)
                    return false;
                var loot2Dump = UnloadLoot.LootItemsInLootHangar();
        ...
        private static int DumpLootIterations => WCFClient.Instance.GetPipeProxy.GetDumpLootIterations(WCFClient.Instance.GUID);

        private static void IncreaseDumpLootIterations() => WCFClient.Instance.GetPipeProxy.IncreaseDumpLootIterations(WCFClient.Instance.GUID);
        ...
        public override void DoWork()
        {
            if (ESCache.Instance.InSpace &&
                ESCache.Instance.ActiveShip.IsPod)
            {
            ...
            switch (_state)
            {
                case DumpLootControllerState.TravelToHomeStation:
                    ...
                    var homeStationId = MissionSettings.AgentToPullNextRegularMissionFrom.StationId;
                ...
                case DumpLootControllerState.MoveLootToShip:
                    ...
                    var loot2Dump = UnloadLoot.LootItemsInLootHangar().OrderByDescending(i => i.IskPerM3);
                    ...
                    foreach (var item in loot2Dump)
                    {
                        ...
                        {
                            if (itemsToMove.Count > 0) // items to move first
                                break;
                            // try to move it partially or skip if the volume of one is more than left free cargo
                            if (item.Volume > freeCargo)
                            {
                                Log($"Item {item.TypeName} Volume {item.Volume} exceeds remaining free cargo {freeCargo}.");
                                continue; // pick the next item in that case
                            }
                            else
                            {
                                var quantityToMove = Convert.ToInt32(Math.Floor((double)freeCargo / item.Volume));
                    ...
                    if (itemsToMove.Count > 0)
                    {
                        ...
                        if (!ESCache.Instance.DirectEve.NoLockedItemsOrWaitAndClearLocks("DumpLootController."))
                            return;
                        ESCache.Instance.CurrentShipsCargo.Add(itemsToMove);
                        LocalPulse = UTCNowAddMilliseconds(3000, 3500);
                        return;
                    }
                ...
                case DumpLootControllerState.MoveLootToItemHangar:
                    ...
                    if (ESCache.Instance.CurrentShipsCargo.Items.Count > 0)
                    {
                        ...
                        if (!ESCache.Instance.DirectEve.NoLockedItemsOrWaitAndClearLocks("DumpLootController!"))
                            return;
                        ESCache.Instance.DirectEve.GetItemHangar().Add(ESCache.Instance.CurrentShipsCargo.Items);
                        LocalPulse = UTCNowAddMilliseconds(3000, 3500);
                        return;
                    }
                    else
                ...
                case DumpLootControllerState.DumpLoot:
                    ...
                    var loot2dump = UnloadLoot.LootItemsInLootHangar().Where(i => !i.IsSingleton).ToList();
