...
{
    public class DrWhoDeadspaceController : BaseController
    {
        #region Constructors

        public DrWhoDeadspaceController()
        {
            IgnorePause = false;
            IgnoreModal = false;
        ...
        public static void ProcessState()
        {
            try
            {
                // Dr Who Deadspace Behavior
                DrWhoDeadspaceBehavior.ProcessState();
            }
            catch (Exception ex)
            {
                Log("Exception [" + ex + "]");
        ...
        public override void DoWork()
        {
            try
            {
                ...
                {
                    if (DebugConfig.DebugCombatMissionsBehavior) Log("DrWhoDeadspaceController: Loading Settings");
                    ...
                    if (Settings.CharacterXmlExists)
                    {
                        if (DateTime.UtcNow > Time.Instance.NextStartupAction)
                        {
                            try
                            {
                                ...
                                {
                                    if (DebugConfig.DebugInteractWithEve) Log("DrWhoDeadspaceController: RunOnce: OpenCargoHold: !OkToInteractWithEveNow");
                                    return;
                                }

                                if (DebugConfig.DebugCombatMissionsBehavior) Log("DrWhoDeadspaceController: RunOnce");
                ...
                {
                    if (DebugConfig.DebugAbyssalDeadspaceBehavior) Log("DrWhoDeadspaceController: if (ESCache.Instance.InWarp)");
                    return;
                }

                if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)
                {
                    Log("DrWhoDeadspaceController: if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)");
                ...
                {
                    Log("DrWhoDeadspaceController: if (ESCache.Instance.InStation && ESCache.Instance.DeactivateScheduleAndCloseAfterNextDock)");
