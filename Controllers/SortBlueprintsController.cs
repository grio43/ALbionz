...
{
    public class SortBlueprintsController : BaseController
    {
        #region Constructors

        public SortBlueprintsController()
        {
        ...
        {
            // Sort Blueprints Behavior
            SortBlueprintsBehavior.ProcessState();
        }
        ...
        public override void DoWork()
        {
            try
            {
                ...
                {
                    if (DebugConfig.DebugCombatMissionsBehavior) Log("SortBlueprintsController: Loading Settings");
                    ...
                    if (Settings.CharacterXmlExists)
                    {
                        if (DateTime.UtcNow > Time.Instance.NextStartupAction)
                        {
                            try
                            {
                                ...
                                {
                                    if (DebugConfig.DebugInteractWithEve) Log("SortBlueprintsController: RunOnce: OpenCargoHold: !OkToInteractWithEveNow");
                                    return;
                                }

                                if (DebugConfig.DebugCombatMissionsBehavior) Log("SortBlueprintsController: RunOnce");
                ...
                {
                    if (DebugConfig.DebugAbyssalDeadspaceBehavior) Log("SortBlueprintsController: if (ESCache.Instance.InWarp)");
                    return;
                }

                if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)
                {
                    Log("SortBlueprintsController: if (ESCache.Instance.InStation && ESCache.Instance.PauseAfterNextDock)");
                ...
                {
                    Log("SortBlueprintsController: if (ESCache.Instance.InStation && ESCache.Instance.DeactivateScheduleAndCloseAfterNextDock)");
