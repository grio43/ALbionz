        ...
        public FindAgentsAction()
        {
            Action = new Action(() =>
            {
                ...
                foreach (var factionId in factions)
                {
                    ...
                    var list = agents.Where(k => !k.System.GetNeighbours(noLowsecWithinXJumps).Any(s => s.IsLowSecuritySpace) // no lowsec system within 2 jumps
                                                 && k.System.GetSecurity() <= 0.64 // max 0.6 sec
                                                 && k.System.IsHighSecuritySpace
                                                 && (highsecIsles || (!highsecIsles && k.System.CalculatePathTo(ESCache.Instance.DirectEve.SolarSystems[30000142],null, true).Item1.Any()))) // include exclude highsec isles
                        .Select(n =>
                            new
                            {
                                origin = n,
                                avgRadius = n.System.GetNeighbours(jumpsAvgRadius).Where(s => s.IsHighSecuritySpace).Select(o => o.Radius).Sum() / n.System.GetNeighbours(jumpsAvgRadius).Count(s => s.GetSecurity() >= 0.45)
