...
namespace EVESharpCore.Controllers.Abyssal
{
    public partial class AbyssalController : AbyssalBaseController
    {
        ...
        {
            var targets = Framework.Entities.Where(e => e.IsPlayer && e.IsAttacking && e.Distance <= _maxRange && !e.IsPod).OrderBy(e => e.OwnerId).ThenBy(e => e.IsWarpScramblingOrDisruptingMe).ToList();
            var droneTarget = targets.FirstOrDefault();
            var weaponTarget = ESCache.Instance.TotalTargetsAndTargeting.Where(e => e.Distance <= Combat.MaxRange && e.IsPlayer && e.IsAttacking && !e.IsPod).FirstOrDefault();
            ...
            foreach (var target in targets)
            {
                ...
                else
                {
                    if (DirectEve.Interval(2000))
                        Log($"Maximum target amount reached. Currently Locked Targets [{ESCache.Instance.TotalTargetsAndTargeting.Count()}] Max Targets [{_maximumLockedTargets}]");
                }
            }

            // Unlock targets
            if (droneTarget != null && !droneTarget.IsTarget && ESCache.Instance.TotalTargetsAndTargeting.Count() >= _maximumLockedTargets)
            {
                var unlockTarget = ESCache.Instance.Targets.OrderBy(e => e.Distance).FirstOrDefault();
            ...
            if (droneTarget != null)
            {
                ...
                var nonAttackingDrones = allDronesInSpace.Where(d => d._directEntity.DroneState != 4 && d._directEntity.DroneState != 1 && d._directEntity.FollowEntity != droneTarget);
                if (nonAttackingDrones.Any())
                {
                    var ids = nonAttackingDrones.Select(e => e.Id).ToList();
                    if (ids.Any())
                    {
                        ...
                        droneTarget.EngageTargetWithDrones(ids);
                    }
                }
            }
            // Attack with guns
            if (weaponTarget != null)
            {
                ...
                //This is handled by AmmoManagementController
                /**
                // reload weapon
                if (weapons.Any(w => w.ChargeQty == 0 || w.ChargeQty > w.MaxCharges))
                {
                    var weap = weapons.FirstOrDefault(w => w.ChargeQty == 0 || w.ChargeQty > w.MaxCharges);
                ...
                **/
                ...
                var weapon = weapons.FirstOrDefault(w => w.ChargeQty > 0);
                            ...
                            if (Framework.Me.CanIWarp(true) && isWarpingByMode.HasValue && !isWarpingByMode.Value)
                            {
                                ...
                                if (star != null)
                                {
                                    if (ESCache.Instance.Star.Distance > _starDist)
                                    {
                                        ...
                                        star._directEntity.WarpTo();
                                        //star._directEntity.WarpTo(ignoreSecurityChecks: true);
                                    }
                                    else
                                    {
            ...
            try
            {
                ...
                if (ESCache.Instance.DirectEve.Me.IsInvuln && AreWeAtTheFilamentSpot && IsAnyOtherNonFleetPlayerOnGrid)
                {
                    ...
                    if (DirectEve.Interval(2000))
                    {
                        ...
                        Log($"Non Fleet player on grid ---- [{string.Join(",", OtherNonFleetPlayersOnGrid.Select(e => e.Name + " -- " + e.TypeName))}]");
                    }
                    ...
                    var medRackExceeded = medRackHeatStatus >= MedRackHeatDisableThreshold;
                    var anyModuleBurnedOut = DirectEve.Modules.Any(m => m.HeatDamagePercent >= heatDamageMaximum_RepairModule);
                    ...
                    var afterBurnerOverheat = Afterburners.All(m => m.IsPendingOverloading || m.IsOverloaded) || !Afterburners.Any() || anyModuleBurnedOut || medRackExceeded;
                    ...
                    if (_pvpActionQueueAction == null && Afterburners.Any())
                    {
                        DateTime timeout = DateTime.UtcNow.AddSeconds(25);
                        ...
                        _pvpActionQueueAction = ActionQueueAction.Run(() =>
                        {
                            ...
                            if (!propModEnabledOnce && Afterburners.Any() && !Framework.Me.IsWarpingByMode)
                            {
                                foreach (var m in Afterburners)
                                {
                            ...
                            var propmod = Afterburners.FirstOrDefault();
                            ...
                            if (propModDisabledOnce && propmod.IsActive && propmod.MillisecondsUntilNextCycle.HasValue)
                            {
                                ...
                                if (propModMillisecondsUntilNextCycle >= 0 && propModMillisecondsUntilNextCycle <= millisecondsNeededToReachThatSpeed + msOffset)
                                {
                                    ...
                                    {
                                        if (star._directEntity.WarpToAtRandomRange())
                                        {
                    ...
                    if (!Afterburners.Any() && DirectEve.Interval(1500, 4000))
                    {
                ...
                var closestStargate = ESCache.Instance.Stargates.OrderBy(e => e.Distance).FirstOrDefault();
                ...
                if (!Framework.Me.IsWarpingByMode && Framework.Me.CanIWarp() && star.Distance > _starDist)
                {
                    ...
                    {
                        star._directEntity.WarpToAtRandomRange();
                    }
                }
