...
namespace EVESharpCore.Controllers
{
    ...
    {
        private AsyncLogQueue _asyncLogQueue;
                    ...
                    if (serviceDict.ContainsKey("infoGatheringSvc") && serviceDict["infoGatheringSvc"].Attribute("state").ToInt() == 4)
                    {
                        ...
                        if (loggedEvents.IsValid)
                        {
                            ...
                            if (loggedEventsDict.Count > 0)
                            {
                                ...
                                {
                                    var openedWndList = loggedEventsDict[(int)InfoManagerConst.infoEventWndOpenedCounters].ToList<PyObject>();
                                    foreach (var i in openedWndList)
                                    {
                                        var currItem = i.ToList<PyObject>();
                                ...
                                {
                                    var wndFocusList = loggedEventsDict[(int)InfoManagerConst.infoEventWndSecondsInFocus].ToList<PyObject>();
        ...
        public void LocalLog(string text, Color? col = null, [CallerMemberName] string memberName = "")
        {
            try
            {
                _asyncLogQueue.Enqueue(new LogEntry(text, memberName, col));
            }
            catch (Exception ex)
            {
