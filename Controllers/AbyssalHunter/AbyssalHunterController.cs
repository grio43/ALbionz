...
namespace EVESharpCore.Controllers.AbyssalHunter
{
    public class AbyssalHunterController : BaseController
    {
        ...
        private DirectUIModule _coverOpsClocks => ESCache.Instance.Modules.FirstOrDefault(i => i.TypeId == 11578)._module;
        ...
        {
            if (Scanner.GetProbes().Count > 0 && Scanner.RecoverProbes())
            {
                Log($"Recovering probes.");
        ...
        private void InvestigateTraces()
        {
            ...
            if (bookmark.DistanceTo(Framework.ActiveShip.Entity) < 150_000)
            {
                ...
                var abyssalRunner = ESCache.Instance.EntitiesNotSelf.FirstOrDefault(e => e.IsPlayer && trace.DirectAbsolutePosition.GetDistance(e._directEntity.DirectAbsolutePosition) < 150_000);
                ...
                try
                {
                    ...
                    wc.DB.Insert<AbyssHunterScans>(new AbyssHunterScans
                    {
                        ...
                        AbyssalRunnerCharacterName = abyssalRunner == null ? "" : Framework.GetOwner(abyssalRunner._directEntity.OwnerId)?.Name ?? "",
                        AbyssalRunnerCharacterId = abyssalRunner?._directEntity.OwnerId ?? 0,
                        AbyssalRunnerShipTypeId = abyssalRunner?._directEntity?.TypeId ?? 0,
                        AbyssalRunnerSigRadius = (int)(abyssalRunner?._directEntity?.SlimSignatureRadius ?? 0),
                        AbyssalRunnerMass = (int)(abyssalRunner?._directEntity?.BallMass ?? 0),
                        AreWeSeen = abyssalRunner != null && !ESCache.Instance.ActiveShip.Entity.IsCloaked,
                        Added = DateTime.UtcNow,
                        X = trace.DirectAbsolutePosition.XCoordinate,
                        Y = trace.DirectAbsolutePosition.YCoordinate,
                        Z = trace.DirectAbsolutePosition.ZCoordinate,
                    });
                    _probeScanState = ProbeScanState.PickNextSig;
                    _visitedSignatureIds[_currentlyVisitingSignatureId] = true;
        ...
        private void TravelToNextSystem()
        {
            ...
            if (Traveler.Destination == null || Traveler.Destination.SolarSystemId != _nextDirectSolarSystem.Id)
            {
                Log($"Set destination to {_nextDirectSolarSystem.Name} Id {_nextDirectSolarSystem.Id}");
                Traveler.Destination = new SolarSystemDestination(_nextDirectSolarSystem.Id);
            }
        ...
        private bool WarpToStar()
        {
            if (!IsAtStar)
            {
                if (DirectEve.Interval(5000, 10000))
                {
                    ...
                    ESCache.Instance.Star._directEntity.WarpToAtRandomRange();
                    return true;
                }
            }
            return false;
        ...
        private bool ReloadCombatProbes()
        {
            ...
            if (probeLauncher.Charge == null || probeLauncher._module.ChargeQty < 8)
            {
        ...
        private void ProbeSystem()
        {
            try
            {
                ...
                var mapViewWindow = ESCache.Instance.probeScannerWindow;
                ...
                if (!Scanner.GetProbes().Any() && _probeScanState != ProbeScanState.InvestigateTrace)
                {
                    var probeLauncher = ESCache.Instance.Modules.FirstOrDefault(m => m.GroupId == 481);
                    if (probeLauncher == null)
                    {
                    ...
                    {
                        Log("Warping to the star before launching probes");
                        WarpToStar();
                        return;
                    }
                ...
                if (Scanner.GetProbes().Count != 8 && RecoverProbes())
                {
                ...
                switch (_probeScanState)
                {
                    ...
                    case ProbeScanState.InitialMaxRangeScan:
                        ...
                        Scanner.MoveProbesTo(ESCache.Instance.Star._directEntity.DirectAbsolutePosition.GetVector());
                        Scanner.ProbeScan();
                        LocalPulse = UTCNowAddSeconds(2, 3);
                    ...
                    case ProbeScanState.InitialSnapShot:
                        ...
                        Scanner.SystemScanResults.ForEach(r =>
                        {
                    ...
                    case ProbeScanState.ScanChosenSig:
                        ...
                        if (Scanner.SystemScanResults.Where(r => r.ScanGroup == ScanGroup.FilamentTrace).All(r => r.Id != _currentSignatureId))
                        {
                            Log($"Signature [{_currentSignatureId}] is not part of the scan results anymore. Removing it from list.");
                            if (_checkedSignatureIds.Any(i => i.Key == _currentSignatureId))
                            {
                                _checkedSignatureIds.Remove(_currentSignatureId);
                            }
                        ...
                        var result = Scanner.SystemScanResults.FirstOrDefault(r => r.ScanGroup == ScanGroup.FilamentTrace && r.Id == _currentSignatureId);
