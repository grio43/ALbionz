...
namespace EVESharpCore.Framework
{
    public class DirectSceneManager : DirectObject
    {
            ...
            try
            {
                ...
                if (currentJob.IsValid)
                {
                    try
                    {
                        ...
                        try
                        {
                            if (cam.Attribute("eyePosition") != null)
                            {
        ...
        public void LogCurrentCameraZoomLevel()
        {
            if (Camera.IsValid)
            {
                if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("if (Camera.IsValid)");
                if (Camera.HasAttrString("minZoom") && Camera.HasAttrString("maxZoom") && Camera.HasAttrString("zoom"))
                {
                    if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("Camera zoom [" + Camera["zoom"].ToFloat() + "] maxZoom [" + Camera["maxZoom"].ToFloat() + "] minZoom [" + Camera["minZoom"].ToFloat() + "]");
                }
            }
        }

        public float CurrentCameraZoomLevel
        {
            get
            {
                if (!DirectEve.Session.IsInSpace)
                ...
                //
                // 1 == fully zoomed out, 0 == fully zoomed in
                //
                return 1;
            }
        }
        ...
        public void SetCameraZoom(float ZoomLevelToSet)
        {
            if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("SetCameraZoom: " + ZoomLevelToSet);
            if (Camera.IsValid)
            {
                if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("if (Camera.IsValid)");
                if (Camera.HasAttrString("minZoom") && Camera.HasAttrString("maxZoom") && Camera.HasAttrString("zoom"))
                {
                    LogCurrentCameraZoomLevel();

                    /** Is this backwards form normal logic?!: oh my...
                     *
                     * def SetMaxZoom(self, value):
                        if value >= self.minZoom:
                            logger.error('Camera: Cannot set maxZoom to a value larger than minZoom: value=%s, %s' % (value, self))
                        else:
                            self.maxZoom = value

                        def SetMinZoom(self, value):
                            if value <= self.maxZoom:
                                logger.error('Camera: Cannot set minZoom to a value smaller than maxZoom: value=%s, %s' % (value, self))
                            else:
                                self.minZoom = value

                        def SetMinMaxZoom(self, minZoom, maxZoom):
                            if minZoom <= maxZoom:
                                logger.error('Camera: Cannot set minZoom to a value smaller than maxZoom: minZoom=%s, maxZoom=%s, %s' % (minZoom, maxZoom, self))
                            self.minZoom = minZoom
                            self.maxZoom = maxZoom
                     * **/

                    //
                    // read above these variables are backwards from what you would expect: they must read like this:
                    // minZoom IN
                    // maxZoom IN
                    //

                    if (ZoomLevelToSet > Camera["minZoom"].ToFloat())
                    {
                        ZoomLevelToSet = Camera["minZoom"].ToFloat();
                    }

                    if (Camera["minZoom"].ToFloat() >= ZoomLevelToSet)
                    {
                        if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("minZoom [" + Camera["minZoom"].ToFloat() + "] > ZoomLevelToSet [" + ZoomLevelToSet + "]");
                        if (ZoomLevelToSet > Camera["maxZoom"].ToFloat())
                        {
                            if (Camera["zoom"].ToFloat() != ZoomLevelToSet)
                            {
                                if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("if (Camera[\"zoom\"].ToFloat() != ZoomLevelToSet)");
                                if (DirectEve.Interval(Time.TenMinutesInMilliSeconds, Time.TenMinutesInMilliSeconds + Time.MinuteInMilliSeconds, DirectEve.Session.SolarSystemId.ToString()))
                                {
                                    if (DebugConfig.DebugReduceGraphicsController) Log.WriteLine("if (DirectEve.Interval(Time.TenMinutesInMilliSeconds, Time.TenMinutesInMilliSeconds + Time.MinuteInMilliSeconds, DirectEve.Session.SolarSystemId.ToString()))");
                                    //disable this for now, much easier to just use the keyboard commands for zooom...
                                    //baseCamera.py SetZoom(1)
                                    //
                                }
                            }
                        }
        ...
        public void EnableZoomHack()
        {
            ...
            if (DirectEve.Session.IsVoidSpace)
                return;

            if (DebugConfig.DebugReduceGraphicsController && DirectEve.Interval(10000)) Log.WriteLine("EnableZoomHack");
            SetCameraZoom((float)1);
        }
            ...
            try
            {
                ...
                var ents = ESCache.Instance.DirectEve.Entities.Where(ise => ise.Distance < 3000000 && (ise.GroupId == (int)Group.SentryGun || ise.HasAnyNonTraversableColliders
                    || (!ignoreAbyssEntities &&
                        ...
                        || ise.IsFilaCloud && !ignoreFilaCouds
