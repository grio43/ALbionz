...
namespace EVESharpCore.Framework
{
    ...
    public class DirectItemAttributes : DirectObject
    {
        ...
        internal DirectItemAttributes(DirectEve directEve, PyObject itemId)
            : base(directEve)
        {
            _chargedAttributes = directEve.GetLocalSvc("godma")
                .Attribute("stateManager")
                .Attribute("chargedAttributesByItemAttribute")
                .DictionaryItem(itemId)
                .ToDictionary<string>();
            _attributes = directEve.GetLocalSvc("godma")
                .Attribute("stateManager")
                .Attribute("attributesByItemAttribute")
                .DictionaryItem(itemId)
                .ToDictionary<string>();
        ...
        public T TryGet<T>(string key)
        {
            if (_chargedAttributes.ContainsKey(key))
            {
                ...
                var charge = DirectEve.GetLocalSvc("godma")
                    .Attribute("stateManager")
                    .Call("GetChargeValue", value.GetItemAt(0), value.GetItemAt(1), value.GetItemAt(2), value.GetItemAt(3));
