...
namespace EVESharpCore.Framework
{
    public class DirectNavigation : DirectObject
    {
        ...
        public bool IsAnyLowsecWayPointOnDestinationPath
        {
            get
            {
                List<long> path = GetDestinationPath();
                foreach (long p in path)
                    if (p < int.MaxValue && DirectEve.SolarSystems.TryGetValue((int) p, out var ss) && ss.GetSecurity() < 0.45)
                        return true;
                return false;
            }
        }
