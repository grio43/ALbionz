...
namespace EVESharpCore.Framework
{
    ...
    public class DirectMe : DirectObject
    {
        ...
        private List<DirectBooster> _boosters;
        public List<DirectBooster> Boosters
        {
            get
            {
                ...
                _boosters = DirectEve.GetLocalSvc("crimewatchSvc").Call("GetMyBoosters").ToList().Select(k => new DirectBooster(DirectEve, k)).ToList();
                return _boosters;
            }
        }
        ...
        public bool IsDownTimeComingSoon()
        {
            ...
            {
                startTime = new TimeSpan(4, 50, 0);
                endTime = new TimeSpan(5, 3, 0);
            }
            ...
            else
        {
            ...
            // bastion .. if you ahve a bastion module detect its active
            // warp bubble .. you can detect th warp disrupt probe (and smart bomb it to death!)
            // siege ..
        ...
        public bool IsAtWar
        {
            get
            {
                ...
                try
                {
                    var allyId = DirectEve.Session.AllianceId ?? -1;
                    var corpId = DirectEve.Session.CorporationId ?? -1;
        ...
        public bool IsInAbyssalSpace(bool skipRemote = false)
        {
            ...
            if (!skipRemote && DirectEve.Interval(2500, 3500, _isInAbyssalSpace.ToString()))
            {
                Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            WCFClient.Instance.GetPipeProxy.SetEveAccountAttributeValue(WCFClient.Instance.GUID, nameof(EveAccount.IsInAbyss), _isInAbyssalSpace.Value);
        ...
        //
        public bool VoidSpaceTimerExist
        {
            get
            {
                var panelSvc = DirectEve.GetLocalSvc("infoPanel");
                if (panelSvc.IsValid)
                {
                    ...
                    if (combatTimerContainer.IsValid)
                    {
                        var abyssalTimer = combatTimerContainer.Attribute("voidSpaceContentExpirationTimer");
                        return !abyssalTimer.IsNone;
                    }
      ...
      : 0;
        ...
        public bool IsSessionChangeActive
        {
            get
            {
                try
                {
                    ...
                    if (panelSvc.IsValid)
                    {
                        var sessionChangeTimer = panelSvc.Attribute("display").ToBool();
                        return sessionChangeTimer;
                    }
        ...
        public bool IsOmegaClone
        {
            get
            {
                ...
                {
                    var b = (int)DirectEve.GetLocalSvc("cloneGradeSvc").Call("GetCloneGrade");
                    _IsOmegaClone = b == 1;
                }

                return _IsOmegaClone ?? false;
            }
        }
        ...
        private static DateTime _lastInWarp;
        ...
        public bool IsWarpingByMode
        {
            get
            {
                if (_isWarpingByMode == null)
                {
                    ...
                    {
                        _lastInWarp = DateTime.UtcNow;
                    }
                }

                if (_lastInWarp.AddMilliseconds(1250) > DateTime.UtcNow)
                    _isWarpingByMode = true;

                if (_isWarpingByMode ?? false)
                    ESCache.Instance.DirectEve.SceneManager.ClearDebugLines();

                return _isWarpingByMode.Value;
            }
        }
        ...
        public DateTime SubTimeEnd
        {
            get
            {
                ...
                {
                    var subTimeEnd = DirectEve.Layers.CharSelectLayer
                ...
                return ESCache.Instance.EveAccount.SubEnd;
            }
        }
        ...
        public float? Wealth
        {
            get
            {
                try
                {
                    var _walletBlance = (float?)DirectEve.GetLocalSvc("wallet").Attribute("wealth");
                    if (_walletBlance != null)
                    {
                        try
                        {
                            if (ESCache.Instance.EveAccount.WalletBalance != (double)_walletBlance)
                                ESCache.Instance.TaskSetEveAccountAttribute(nameof(EveAccount.WalletBalance), (double)_walletBlance);

                            return _walletBlance ?? 0;
                        }
                        catch (Exception ex)
                        {
                    ...
                    if (DirectEve.Interval(5000) && ESCache.Instance.InStation) ESCache.Instance.DirectEve.ExecuteCommand(DirectCmd.OpenWallet);
                    if (DirectEve.Interval(5000)) Log.WriteLine("_walletBlance is null");
                    return null;
                }
                catch (Exception ex)
                {
        ...
        public List<DirectDgmEffect> GetAllBoosterEffects()
        {
            return Boosters.SelectMany(k => GetBoosterEffects(k)).ToList();
        }
        ...
        public void SetSafety(SafetyLevel sl)
        {
            ...
            else if (sl == SafetyLevel.ShipSafetyLevelNone && (DirectEve.Me.IsOmegaClone))
            {
                DirectEve.ThreadedCall(DirectEve.GetLocalSvc("crimewatchSvc")["SetSafetyLevel"],
        ...
        public double? DistanceFromMe(Vec3 pos)
        {
            ...
            var curX = DirectEve.ActiveShip.Entity.DirectAbsolutePosition.XCoordinate;
            var curY = DirectEve.ActiveShip.Entity.DirectAbsolutePosition.YCoordinate;
            var curZ = DirectEve.ActiveShip.Entity.DirectAbsolutePosition.ZCoordinate;
        ...
        {
            var DistanceFromMeValue =  DistanceFromMe(new Vec3(x, y, z));
            return DistanceFromMeValue ?? 0;
        }
